...
class DatabaseService {
        ...
        if (supabase != null) {
          ...
          return response.map<Product>((p) {
            ...
            if (product.imageUrl != null && !product.imageUrl!.contains('http')) {
              return product.copyWith(imageUrl: supabase.storage
                  .from('product-images')
  ...
  Future<void> updateProduct(Product product) async {
    ...
    if (await _isOnline()) {
      try {
        final supabase = await _getSupabaseClient();
        if (supabase != null) {
          await supabase.from('products').update(productJson).eq('id', product.id);
        }
      } catch (e) {
  ...
  Future<void> updateProductWithImage({
    ...
    try {
      ...
      if (await _isOnline() && newImageFile != null && updatedImageUrl != null && !updatedImageUrl.startsWith('http')) {
        final supabase = await _getSupabaseClient();
        if (supabase != null) {
          final fileName = basename(updatedImageUrl);
          await supabase.storage.from('product-images').upload(fileName, newImageFile);
